...
locals {
  ...
  codebuil_security_group = data.terraform_remote_state.shd_bootstrap.outputs["shared"].codebuild_bootstrap_security_group_id
	more_tags = {
		abac_operator = "${data.terraform_remote_state.lz_adfs_account_roles.outputs.abac_codes.are} ${data.terraform_remote_state.lz_adfs_account_roles.outputs.abac_codes.cai} ${data.terraform_remote_state.lz_adfs_account_roles.outputs.abac_codes.itsma}"
    abac_admin = "${data.terraform_remote_state.lz_adfs_account_roles.outputs.abac_codes.are} ${data.terraform_remote_state.lz_adfs_account_roles.outputs.abac_codes.cai} ${data.terraform_remote_state.lz_adfs_account_roles.outputs.abac_codes.itsma}"
	}
}
